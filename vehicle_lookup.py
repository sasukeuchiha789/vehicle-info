...
def loading_animation():
    console.print("\n[bold green]Initializing SASUKE UCHIHA...[/bold green]\n")
...
def banner():
    console.rule()
    console.print(Align.center(f"[bold red]SASUKE UCHIHA[/bold red]  •  [yellow]{VERSION}[/yellow]"))
    console.print(Align.center("[green]Created by: SASUKE • YouTube: MADARA 😈👿[/green]"))
    console.rule()
    console.print(
        Panel(
...
def fetch_vehicle_data(rc):
    ...
    try:
        resp = requests.get(
            url,
            timeout=15,
            headers={"User-Agent": "SASUKE UCHIHA/PRO (by SASUKE)"},
        )
...
def print_results(rc, data, from_cache):
    tool_info = f"[bold magenta]SASUKE UCHIHA[/bold magenta]  •  [bold magenta]by SASUKE[/bold magenta]"
